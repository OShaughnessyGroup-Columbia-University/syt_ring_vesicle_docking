...
void normalize(double r[], double rn[])
{
	...
	double z;
	z=norm(r);
	if(z>1e-10) { for(i=0; i<3; i++) rn[i]=r[i]/z; }
...
{
	int i;
	for(i=0; i<3; i++) r4[i]=r1[i]+r2[i]+r3[i];
}
...
{
	int i;
	for(i=0; i<3; i++) r5[i]=r1[i]+r2[i]+r3[i]+r4[i];
}
...
{
	int i;
	for(i=0; i<3; i++) r6[i]=r1[i]+r2[i]+r3[i]+r4[i]+r5[i];
}
...
{
	int i;
	for(i=0; i<3; i++) r7[i]=r1[i]+r2[i]+r3[i]+r4[i]+r5[i]+r6[i];
}
...
void vecdiv(double r1[], double r, double r2[])	// r[3]
{
	...
	else for(i=0; i<3; i++) r2[i]=r1[i]/r;
}
...
{
	int i;
	for(i=0; i<3; i++) r3[i]=(r1[i]+r2[i])/2.0;
}
...
{
	int i;
	for(i=0; i<3; i++) r4[i]=(r1[i]+r2[i]+r3[i])/3.0;
}
...
{
	int i;
	for(i=0; i<4; i++) r5[i]=(r1[i]+r2[i]+r3[i]+r4[i])/4.0;
}
...
double distance(double r1[], double r2[])
{
	...
	double r;
	r=0.0;
	for(i=0; i<3; i++) r+=pow(r1[i]-r2[i],2);
	r=sqrt(r);
	return r;
}
...
*/
///* simplified version
	z = Xor(SegSegLeft2D(r1,r2,r3), SegSegLeft2D(r1,r2,r4)) && 
		Xor(SegSegLeft2D(r1,r3,r4), SegSegLeft2D(r2,r3,r4));
//*/
	return z;
}
